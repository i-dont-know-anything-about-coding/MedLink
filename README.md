# StockSync — Frontend (MVP v4, AI Expiry Queue + Emergency Search + Real Delivery + Google Maps)
...
5. เปิดไฟล์ **`.env.local`** ที่ root ของโปรเจกต์ frontend (สร้างไฟล์นี้ถ้ายังไม่มี — อยู่ระดับเดียวกับ
   `package.json`, path เต็ม: `StockSync-main/.env.local`) แล้วเพิ่มบรรทัด:
...
## Backend ที่แก้ไปด้วยรอบนี้ (ดู `Backend_Stocksync-main/`)
