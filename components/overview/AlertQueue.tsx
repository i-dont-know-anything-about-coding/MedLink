...
interface AlertQueueProps {
  ...
  onAction: (item: AlertQueueItem) => void;
}

export default function AlertQueue({ items, onAction }: AlertQueueProps) {
  // เรียงรายการที่ตรวจพบล่าสุดไว้บนสุด (ใกล้เวลาปัจจุบันที่สุดก่อน)
  ...
  if (sorted.length === 0) {
    return (
      <div className="flex h-full items-center justify-center p-6 text-center text-[13px] text-text-lo">
        ไม่มีรายการแจ้งเตือนวิกฤตในขณะนี้ — สต็อกทุกแห่งอยู่ในระดับปลอดภัย
      </div>
    );
  }
